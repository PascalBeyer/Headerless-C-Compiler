...
#define GMEM_FIXED 0
    wchar_t *preped_command_line = (wchar_t *)GlobalAlloc(GMEM_FIXED, sizeof(wchar_t) * (command_line_size + 1));
    int argc = windows_parse_command_line__internal(command_line, command_line_size, preped_command_line);
    
    wchar_t **argv = (wchar_t **)GlobalAlloc(GMEM_FIXED, (argc + 1) * sizeof(wchar_t *));
