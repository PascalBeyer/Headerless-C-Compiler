    ...
    if(token){
        ...
        if(stage == COMPILE_STAGE_tokenize_files){
            ...
            *copied_token = *token;
            ...
            if(compilation_unit && compilation_unit->tokens.data <= token && token < compilation_unit->tokens.data + compilation_unit->tokens.size){
...
func int should_report_warning_for_token(struct context *context, struct token *token){
    ...
    if(globals.cli_options.report_warnings_in_system_includes || context->in_error_report || !token) return true;
    
    struct compilation_unit *compilation_unit = context->current_compilation_unit;
    
    if((globals.compile_stage != COMPILE_STAGE_tokenize_files) && (token->location_index != -1) && (token->location_index & 0x80000000)){
        // 
        // If the token was generated by a define, we need to figure out the compilation unit,
        // to find the `macro_expansion_record` in `get_location_of_token`.
        // 
    ...
    s32 file_index = token_get_file_index(compilation_unit, token);
    
    return !globals.file_table.data[file_index]->is_system_include;
}
